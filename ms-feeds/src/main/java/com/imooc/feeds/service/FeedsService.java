...
public class FeedsService {
    ...
    public void create(Feeds feeds, String accessToken) {
        ...
        followers.forEach(follower -> { //follower是每一个粉丝
            String key = RedisKeyConstant.following_feeds.getKey() + follower;
            //ZSet是sorted Set， 可以基于分数进行排序，这里的分数就是时间
            //在redis中为每一个粉丝维护一个feeds的有序集合，集合中存放的是它的关注者发送的feed。因此，当一个人添加一条feed的时候，我们就通过Redis将这一条feed存到Redis中它的每一个粉丝的feeds集合中
